...
class EvacRes():
    ...
    def __init__(self, res_data, initial_docks):
        ...
        self.current_passengers = 0 # models the currentnumber of passengers
        self.current_operating_cost = 0
    ...
    def update_route_time(self, island_docks, mainland_docks):
        ...
        for i in range(len(self.route) - 1):
            if self.route[i+1].name in [x.name for x in island_docks]:
                ...
                route_time = route_time + (self.loading_time + (self.route[i].distances['Distance'][self.route[i].distances['Destination'] == self.route[i+1].name]/self.vmax) * 60).values[0] # UPDATE THE VMAX HERE
            # print(route_time)
            # update the dock record of visits
            self.route[i+1].resource_visits.append([route_time, i, self, self.route[i], self.route[i].location])
            # if self.passengers_route[i] == 0:
            #     route_time = route_time + (self.loading_time + (self.route[i].distances['Distance'][self.route[i].distances['Destination'] == self.route[i+1].name]/self.vmax) * 60).values[0]
            # else:
        ...
        self.current_operating_cost = (self.current_route_time - self.time_to_availability)/60 * self.operating_cost # calculate current operating cost

    def update_route_time_no_visit_add(self, island_docks, mainland_docks):
        
        # a function to update the current route time
        route_time = self.time_to_availability
        # print(self.passengers_route)
        for i in range(len(self.route) - 1):
            if self.route[i+1].name in [x.name for x in island_docks]:
                route_time = route_time + (self.loading_time + (self.route[i].distances['Distance'][self.route[i].distances['Destination'] == self.route[i+1].name]/self.vmax) * 60).values[0]
            elif self.route[i+1].name in [x.name for x in mainland_docks]:
